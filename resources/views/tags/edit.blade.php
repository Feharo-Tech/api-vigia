...
@section('content')
    <div class="max-w-2xl mx-auto">
        <x-subpage-title>Editar Tag</x-subpage-title>
        
        <form action="{{ route('tags.update', $tag) }}" method="POST" class="bg-white rounded-lg shadow p-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-12 gap-4">
                    ...
                    <div class="col-span-4">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <span id="color_text" class="mt-1 font-bold text-sm text-gray-600">{{ old('color', $tag->color) }}</span>
                        </div>
                        @error('color')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
