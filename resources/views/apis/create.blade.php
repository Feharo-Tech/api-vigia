        ...
        <form action="{{ route('apis.store') }}" method="POST" class="bg-white rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        <label for="expected_status_code" class="block text-sm font-medium text-gray-700">Status Esperado *</label>
                        <input type="number" name="expected_status_code" id="expected_status_code" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                               placeholder="200"
                               value="{{ old('expected_status_code', 200) }}">
                    ...
                    <div>
                        <label for="check_interval" class="block text-sm font-medium text-gray-700">Intervalo de Verificação *</label>
                        <select name="check_interval" id="check_interval" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @foreach($intervals as $value => $label)
                                <option value="{{ $value }}" {{ old('check_interval', 5) == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="flex items-end">
                        <div class="flex items-center">
                            ...
                            <label for="is_active" class="ml-2 block text-sm font-medium text-gray-700">Monitoramento ativo</label>
                        </div>
                    </div>
                    ...
                    <div class="flex items-end">
                        <div class="flex items-center">
                            ...
                            <label for="should_notify" class="ml-2 block text-sm font-medium text-gray-700">Habilitar Notificação</label>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="error_threshold" class="block text-sm font-medium text-gray-700">Sensibilidade (erros consecutivos) *</label>
                    ...
                    <div>
                        <label for="timeout_threshold" class="block text-sm font-medium text-gray-700">Timeout máximo (segundos) *</label>
                ...
                <div>
                    <label for="expected_response" class="block text-sm font-medium text-gray-700">Conteúdo Esperado na Resposta (opcional)</label>
                    ...
                    <p class="mt-1 text-sm text-gray-500">Se preenchido, o sistema verificará se a resposta contém este texto.</p>
            ...
            <div class="mt-6 flex justify-end">
                <a href="{{ route('apis.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded mr-2">
                    Cancelar
                </a>
